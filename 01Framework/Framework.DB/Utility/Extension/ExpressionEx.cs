...
namespace JSHC.IFramework.Utility.Extension
{
    ...
    {
        public static IDictionary<string, object> ToDictionary<T>(this Expression<Func<T, T>> expression)
        {
            var dict = new Dictionary<string, object>();
            if (expression.Body is MemberInitExpression)
            {
                var members = expression.Body as MemberInitExpression;
                foreach (var binding in members.Bindings)
                {
                    var key = binding.Member.Name;
                    var ag = binding as MemberAssignment;
                    if (ag == null) continue;
                    var con = ag.Expression as ConstantExpression;
                    if (con == null) continue;
                    dict.Add(key, con.Value);
                }
            }
            return dict;
        }
